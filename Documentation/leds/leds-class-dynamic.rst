...
``frame`` (write-only, binary)
    ...
    segmented lighting bars. Each complete write is forwarded to the driver as
    one frame payload and must not exceed the size published on the binary
    attribute (at most 65536 bytes). Kernfs may deliver large payloads in
    ``PAGE_SIZE`` chunks; chunked writes must fill the advertised size before
